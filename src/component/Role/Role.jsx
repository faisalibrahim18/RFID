...
const Role = () => {
  ...
  const [editFrontEndPrivileges, setEditFrontEndPrivileges] = useState({});
  const [editBackEndPrivileges, setEditBackEndPrivileges] = useState({});
  ...
  const handleShowEditKategori = async () => {
    try {
      const token = localStorage.getItem("token");
      ...
      const response = await axios.put(
        `http://localhost:9000/api/v1/rfid/role/${editRole._id}`,
        {
          name: editRoleName,
          privileges: newPrivileges,
        },
  ...
  const handleDeleteKategori = async (kategoriId) => {
    try {
      ...
      if (result.isConfirmed) {
        const response = await axios.delete(`http://localhost:9000/api/v1/rfid/role/${kategoriId}`, {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleShowEditKategoriButton = (kategori) => {
    const roleData = role.find((item) => item._id === kategori);
    if (roleData) {
      setEditRole(roleData);
      setEditRoleName(roleData.name);

      const frontEndPrivileges = {};
      const backEndPrivileges = {};
      1;
      roleData.rolePrivileges.forEach((item, index) => {
        console.log(`Index ${index}: allow = ${item.allow}`);
      });

      setEditFrontEndPrivileges(frontEndPrivileges);
      setEditBackEndPrivileges(backEndPrivileges);
      setIsEditing(true);
      setShowAdd(true);
    }
  };
                  ...
                  {role.map((item, index) => (
                    <tr key={item._id} className="border-b text-center text-gray-600">
                      ...
                      <td className="whitespace-nowrap px-6 py-4">
                        <button onClick={() => handleShowEditKategoriButton(item._id)} className="m-3">
                          <i className="fa-solid fa-pen-to-square text-[#96CDF4] hover:text-blue-400"></i>
                        </button>
                        <button onClick={() => handleDeleteKategori(item._id)} className="m-3">
            ...
            <div className="relative w-auto my-6 mx-auto max-w-sm">
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                <div className="flex items-start justify-between p-5 border-b border-solid border-slate-200 rounded-t">
                  <h3 className="text-2xl font-semibold">{editRole ? "Edit Role" : "Add Role"}</h3>
                  <button
                ...
                <div className="relative p-6 flex-auto">
                  <form className="w-full">
                    <div className="mb-4">
                      <label htmlFor="Nama Kategori" className="text-sm font-semibold text-gray-800">
                        Nama
                      </label>
                      <input
                        type="text"
                        ...
                        onChange={(e) =>
                          isEditing ? setEditRoleName(e.target.value) : setNewRoleName(e.target.value)
                        }
                      />
                    </div>
                            ...
                            <div className="mb-4" key={item._id}>
                              <div className="flex flex-shrink">
                                ...
                                <div className="w-1/2 pl-36">
                                  <Switch
                                    defaultChecked
                                    onChange={() => handlePrivilegeToggle(item._id, "frontEnd")}
                                    className={`${
                                      frontEndPrivileges[item._id] ? "bg-green-400" : "bg-gray-300"
                            ...
                            <div className="mb-4" key={item._id}>
                              <div>
                                ...
                                <div className="inline-flex pl-10 items-center">
                                  <Switch
                                    defaultChecked
                                    onChange={() => handlePrivilegeToggle(item._id, "backEnd")}
                                    className={`${
                                      backEndPrivileges[item._id] ? "bg-green-400" : "bg-gray-300"
                                    } relative inline-flex h-[24px] w-[48px] shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75`}
                                  >
                                    <span
                                      aria-hidden="true"
                    ...
                    <div className="flex justify-center pt-10">
                      <button
                        ...
                        onClick={isEditing ? handleShowEditKategori : handleAddRole}
                      >
                        {isEditing ? "Save Changes" : showAdd ? "Save" : "Add Role"}
                      </button>
                    </div>
                  </form>
                </div>
